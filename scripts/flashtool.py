...
def is_coral_micro_connected(serial_number):
  return True if EnumerateCoralMicro() else False

# --- MODIFIED: Bruteforce detection, ignore serial strings ---
def is_elfloader_connected(serial_number):
  if usb.core.find(idVendor=ELFLOADER_VID, idProduct=ELFLOADER_PID):
    return True
  if usb.core.find(idVendor=ELFLOADER_VID, idProduct=ELFLOADER_OLD_PID):
    return True
  return False
...
def FindSerialPortForDevice(serial_number=None):
  for port in serial.tools.list_ports.comports():
    try:
      # Stripped strict checking here too just in case
      if '18D1' in port.hwid:
          return port.device
    except ValueError:
      pass
  return None
...
# --- MODIFIED: Bruteforce open, ignore serial strings ---
@contextlib.contextmanager
def OpenHidDevice(vid, pid, serial_number=None):
  h = hid.device()
  print('OpenHidDevice vid={:x} pid={:x} ... (FORCED)'.format(vid, pid))
  for _ in range(round(OPEN_HID_RETRY_TIME_S / OPEN_HID_RETRY_INTERVAL_S)):
    try:
      h.open(vid, pid) # STRIPPED SERIAL NUMBER ARGUMENT
      print('OpenHidDevice vid={:x} pid={:x} opened'.format(vid, pid))
      try:
        yield h
      finally:
        h.close()
...
def main():
  ...
  basic_group.add_argument('--build_dir', '-b', type=str, default=BUILD_DIR)
  ...
  basic_group.add_argument('--subapp', type=str, required=False)
  basic_group.add_argument('--ram', dest='ram', action='store_true')
  basic_group.add_argument('--noreset', dest='noreset', action='store_true')
  basic_group.add_argument('--serial', type=str, required=False)

  app_elf_group.add_argument('--list', dest='list', action='store_true')
  app_elf_group.add_argument('--ums', dest='ums', action='store_true')

  parser.add_argument('--elfloader_path', type=str, required=False, help=argparse.SUPPRESS)
  parser.add_argument('--flashloader_path', type=str, required=False, help=argparse.SUPPRESS)

  network_group = parser.add_argument_group('Board network settings')
  network_group.add_argument('--usb_ip_address', type=str, required=False, default='10.10.10.1')
  network_group.add_argument('--dns_server', type=str, required=False, default=None)
  network_group.add_argument('--ethernet_config', type=str, required=False, default=None)
  network_group.add_argument('--wifi_ssid', type=str, required=False, default=None)
  network_group.add_argument('--wifi_psk', type=str, required=False, default=None)
  network_group.add_argument('--wifi_config', type=str, required=False, default=None)
  network_group.add_argument('--wifi_country', type=str, required=False, default=None, help=argparse.SUPPRESS)
  network_group.add_argument('--wifi_revision', type=int, required=False, default=None, help=argparse.SUPPRESS)
  network_group.add_argument('--ethernet_speed', type=int, choices=[10, 100], required=False, default=None)

  debug_group = parser.add_argument_group('Debugging options')
  parser.add_argument('--toolchain', type=str, required=False, help=argparse.SUPPRESS)
  debug_group.add_argument('--debug', dest='debug', action='store_true')
  debug_group.add_argument('--jlink_path', type=str, default='/opt/SEGGER/JLink')

  advanced_group = parser.add_argument_group('Advanced options')
  advanced_group.add_argument('--cached', dest='cached', action='store_true')
  advanced_group.add_argument('--data_dir', type=str, required=False, nargs='*', help=argparse.SUPPRESS)
  advanced_group.add_argument('--noprogram', dest='noprogram', action='store_true')
  advanced_group.add_argument('--nodata', dest='nodata', action='store_true')
  parser.add_argument('--arduino', dest='arduino', action='store_true', help=argparse.SUPPRESS)
  ...
  for elfloader in EnumerateElfloader():
    # Attempting to safely pull serial numbers for the list, but won't crash if they fail
    try:
        serial_list.append(elfloader.get('serial_number', 'UNKNOWN'))
    except:
        pass
  ...
  # --- MODIFIED: Hardcode serial_number to None to bypass validation checks ---
  serial_number = None
